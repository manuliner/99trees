...
- **REQ-003:** WHEN a team scans a valid task QR for the pending field THEN the system SHALL transition the turn to `scanned` and attach the task.
...
- **REQ-011:** WHEN a player switches locale THEN player UI copy SHALL follow DE or EN; task content SHALL resolve bilingual fields for the active locale.
...
- **REQ-NF-003:** Admin and crew UI SHALL remain English; player flows SHALL support DE default and EN.
