...
name: docs-sync
description: >
  Surgical factual patches to AGENTS.md, web/**/README.md, and .vibe/docs/ after code changes.
...
| Module | `web/**/README.md`, `web/README.md` |
| Concepts | `.vibe/docs/*.md` |
| Rules | `.cursor/rules/*.mdc` if conventions changed |

Human-only (unless user asks): `docs/SCOPE.md`, `docs/DEPLOY.md`, `README.md`.
...
3. Remove dead paths (deleted routes, components, URLs).
4. Run verify:
...
- [ ] No legacy `ARCHITECTURE.md` / `docs/AGENTS_*` references reintroduced
- [ ] API facts live in `web/server/api/**/README.md`, not `AGENTS.md`
- [ ] `AGENTS.md` ≤200 lines
