...
- Boundaries, turn flows, cross-cutting architecture → `.vibe/docs/architecture.md` (or run **docs-concepts**).
- “Where to change X” for a module → that module’s `web/**/README.md` (regen via **docs-update** if large drift).
- Durable architectural decision → ADR under `docs/adr/`.
- Single function contract → inline JSDoc only.

## Anti-patterns

- Do not recreate `ARCHITECTURE.md` or `docs/AGENTS_*.md`.
